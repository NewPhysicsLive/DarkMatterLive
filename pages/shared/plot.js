...
function getPlotBounds() {
  ...
  // 1. Select all line and area paths created inside dataLayer
  // (Adjust the selector if your elements have a different container class)
  d3.selectAll(".data-layer path.line, .data-layer path.area").each(function () {
    const element = d3.select(this);

    // 2. Check the computed display style or inline style
    const displayStyle = element.style("display");

    // If it's explicitly hidden, skip it
    if (displayStyle === "none") {
      return;
    }

    // 3. Retrieve the D3 bound data array: [{x: 1, y: 2}, ...]
    const data = element.datum();

    if (data && Array.isArray(data)) {
      data.forEach(d => {
        // Guard against missing values or non-numbers
...
async function getCurveList() {
  try {
    const res = await fetch(`/data/${MODEL_NAME}/curves.json`);
...
async function loadData(curve_list) {
  const results = await Promise.all(
    curve_list.map(async (curve) => {
      const res = await fetch(`/data/${MODEL_NAME}/${curve}`);
      return res.json();
    })
  );
...
plotTitle.append("xhtml:div").html(
  katex.renderToString(`${PLOT_TITLE}`, {
    throwOnError: false,
  })
...
foX.append("xhtml:div").html(
  katex.renderToString(`${PLOT_X_TITLE}`, {
    throwOnError: false,
  })
).style("max-width", "fit-content")
 .style("margin-inline", "auto");
...
foY.append("xhtml:div").html(
    katex.renderToString(`${PLOT_Y_TITLE}`, {
     throwOnError: false,
  })
